...
MIN_GOOD_MATCHES   = 12
SAME_IMAGE_IOU_THRESHOLD = 0.50
LEGACY_TEMPLATE_THRESHOLD = 0.95
...
.det-badge.saved  { border-color: rgba(255,210,120,.35); color: #eac36d; }
...
def normalize_correction_entry(entry: object, fallback_id: str) -> dict | None:
    ...
    source_bbox = normalize_bbox(entry.get("source_bbox") or entry.get("bbox"))
    ...
    return {
        ...
        "source_image_hash": str(
            entry.get("source_image_hash") or entry.get("image_hash") or ""
        ).strip(),
...
def normalize_bbox(bbox: object) -> list[int] | None:
...
def hash_image_np(img_np: np.ndarray) -> str:
    img_c = np.ascontiguousarray(img_np)
    digest = hashlib.sha256()
    digest.update(str(tuple(img_c.shape)).encode("utf-8"))
    digest.update(img_c.tobytes())
    return digest.hexdigest()


def bbox_iou(box_a: list | tuple, box_b: list | tuple) -> float:
    a = normalize_bbox(box_a)
    b = normalize_bbox(box_b)
...
def locate_crop_in_image(img_np: np.ndarray,
                         crop_np: np.ndarray) -> tuple[list[int] | None, float]:
    if crop_np is None or crop_np.size == 0:
        return None, 0.0

    img_gray = cv2.cvtColor(img_np, cv2.COLOR_RGB2GRAY)
    crop_gray = cv2.cvtColor(crop_np, cv2.COLOR_BGR2GRAY)
    crop_h, crop_w = crop_gray.shape[:2]
    img_h, img_w = img_gray.shape[:2]
    if crop_h <= 0 or crop_w <= 0 or crop_h > img_h or crop_w > img_w:
        return None, 0.0

    result = cv2.matchTemplate(img_gray, crop_gray, cv2.TM_CCOEFF_NORMED)
    _, max_val, _, max_loc = cv2.minMaxLoc(result)
    x1, y1 = max_loc
    return [x1, y1, x1 + crop_w, y1 + crop_h], float(max_val)
...
def apply_correction_memory(img_np: np.ndarray, detections: list,
    ...
    for i, det in enumerate(detections):
        ...
        if best_visual_score >= MIN_GOOD_MATCHES and best_visual_label:
...
def apply_same_image_corrections(img_np: np.ndarray,
                                 detections: list,
                                 corrections: list,
                                 match_sources: list | None = None) -> tuple[list, list]:
    if match_sources is None:
        match_sources = [None] * len(detections)

    image_hash = hash_image_np(img_np)
    seen = set()

    for entry in corrections:
        label = str(entry.get("corrected_label", "")).strip()
        if not label or label.lower() == "unknown":
            continue

        source_hash = str(entry.get("source_image_hash", "")).strip()
        source_bbox = normalize_bbox(entry.get("source_bbox"))
        crop_b64 = str(entry.get("crop_b64", ""))
        crop_sig = hashlib.sha1(crop_b64.encode("utf-8")).hexdigest() if crop_b64 else ""
        dedupe_key = (source_hash, tuple(source_bbox or []), label, crop_sig)
        if dedupe_key in seen:
            continue
        seen.add(dedupe_key)

        saved_bbox = source_bbox
        if source_hash and source_hash != image_hash:
            continue

        if saved_bbox is None and crop_b64:
            stored_np = b64_to_np(crop_b64)
            if stored_np is not None:
                candidate_bbox, score = locate_crop_in_image(img_np, stored_np)
                if candidate_bbox is not None and score >= LEGACY_TEMPLATE_THRESHOLD:
                    saved_bbox = candidate_bbox
        ...
        for idx, det in enumerate(detections):
            score = bbox_iou(det.get("bbox", []), saved_bbox)
            if score > best_iou:
                best_iou = score
                best_idx = idx

        if best_idx is not None and best_iou >= SAME_IMAGE_IOU_THRESHOLD:
            detections[best_idx]["bbox"] = saved_bbox
            detections[best_idx]["label"] = label
            if entry.get("original_class"):
                detections[best_idx]["original_class"] = str(entry.get("original_class", ""))
            if best_idx < len(match_sources):
                match_sources[best_idx] = "saved"
        else:
            detections.append({
                "cls_id": -1,
                "conf": 1.0,
                "bbox": saved_bbox,
                "original_class": str(entry.get("original_class") or "manual"),
                "label": label,
            })
            match_sources.append("saved")
...
with col_left:
    ...
    if uploaded_file is None:
        ...
        for k in ("detections", "img_np", "pil_img", "match_sources", "save_status", "image_hash"):
...
with col_right:
    ...
    if run_btn and uploaded_file is not None:
        ...
        with st.spinner("Checking correction memory…"):
            ...
            detections, match_sources = apply_same_image_corrections(
                img_np, detections, corrections, match_sources)
    ...
    if save_btn and has_dets:
        ...
        image_hash = hash_image_np(img_np) if img_np is not None else ""
        saved      = 0
        for det in detections:
            ...
            corrections.append({
                ...
                "source_bbox":     normalize_bbox(det.get("bbox", [])) or [],
            ...
            for i, det in enumerate(detections):
                ...
                with c_inp:
                    ...
                    if src == "visual":
                        ...
                        st.markdown('<span class="det-badge saved">saved box</span>',
                                    unsafe_allow_html=True)
                    elif src == "name":
